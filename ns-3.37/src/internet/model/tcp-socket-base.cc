...
bool m_isHandoverInProgress; // 用于表示切换状态的变量
bool m_forceSmallWindow;     // 控制窗口缩放
...
void
TcpSocketBase::SetHandoverStatus(bool status)
{
    m_isHandoverInProgress = status;
    // std::cout << "==================trans true=============" << std::endl;
}
...
TcpSocketBase::DoForwardUp(Ptr<Packet> packet, const Address& fromAddress, const Address& toAddress)
{
    ...
    // std::cout << "-------DoForwardUp-------------" << tcpHeader.GetWindowSize() << std::endl;
    ...
    if (tcpHeader.GetFlags() & TcpHeader::SYN)
    {
        ...
        // std::cout << "-------DoForwardUp-------------" << m_rWnd << std::endl;
...
TcpSocketBase::ProcessEstablished(Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
    ...
    if (tcpflags == TcpHeader::ACK)
    {
        ...
        else if (tcpHeader.GetAckNumber() > m_tcb->m_highTxMark)
        {
            ...
            {
                SendSplitAck(m_tcb->m_rxBuffer->NextRxSequence(),
                             m_highTxAck,
                             TcpHeader::ACK | TcpHeader::ECE);
            ...
            {
                SendSplitAck(m_tcb->m_rxBuffer->NextRxSequence(), m_highTxAck, TcpHeader::ACK);
            }
        }
        else
        {
        ...
        else
        {
            ...
            if (!m_noDelay && UnAckDataCount() > 0 && availableData < m_tcb->m_segmentSize)
            {
                NS_LOG_DEBUG("Invoking Nagle's algorithm for seq "
                             ...
                             << ". Wait to send.");
                std::cout << "Invoking Nagle's algorithm for seq " << next
                          << ", SFS: " << m_txBuffer->SizeFromSequence(next) << ". Wait to send."
                          << std::endl;
                break;
            }
...
{
    // std::cout << "m_forceSmallWindow:" << m_forceSmallWindow << std::endl;
    ...
    else
    {
        ...
        // if (m_forceSmallWindow)
        if (m_isHandoverInProgress)
        {
            if (m_endPoint->GetLocalAddress() == Ipv4Address("7.0.0.2"))
            {
                NS_LOG_LOGIC("Forcing small window");
                // w >>= 7;
                w = static_cast<uint32_t>(1500);
                //                 std::cout << "window size:" << w << std::endl;
                // return static_cast<uint16_t>(w);  // 强制small窗口
                Simulator::Schedule(MilliSeconds(50), [=]() { m_isHandoverInProgress = false; });
            }
            else
            {
                w = static_cast<uint32_t>(m_tcb->m_rxBuffer->MaxRxSequence() -
    ...
    // std::cout << "advwin" << w << std::endl;

    return static_cast<uint16_t>(w);
}

SequenceNumber32 lastAckSeq{0};

void
TcpSocketBase::SendSplitAck(SequenceNumber32 currentReceivedBytes,
                            SequenceNumber32 lastAckedBytes,
                            uint8_t flags)
{
    NS_LOG_FUNCTION(this << currentReceivedBytes << lastAckedBytes);

    // Only split if there are new bytes to acknowledge
    if (lastAckedBytes < currentReceivedBytes)
    {
        uint32_t count = m_isHandoverInProgress ? m_splitCountBad : m_splitCountGood;
        uint32_t newBytes = currentReceivedBytes.GetValue() - lastAckedBytes.GetValue();

        for (uint32_t i = 1; i <= count; ++i)
        {
            uint32_t step = (uint32_t)(((double)newBytes * i) / count);
            SequenceNumber32 ackNumber = lastAckedBytes + step;

            if (i == count)
            {
                ackNumber = currentReceivedBytes;
            }

            Ptr<Packet> p = Create<Packet>();
            TcpHeader header;
            header.SetFlags(flags); // Use passed flags
            ...
            if (m_endPoint)
            {
                m_tcp->SendPacket(p,
                                  ...
                                  m_endPoint->GetPeerAddress(),
                                  m_boundnetdevice);
                NS_LOG_DEBUG("Sent Split ACK " << i << "/" << count << ": " << ackNumber << " to "
                                               << m_endPoint->GetPeerAddress());
            }
            else
            {
...
TcpSocketBase::ReceivedData(Ptr<Packet> p, const TcpHeader& tcpHeader)
{
    ...
    if (m_isHandoverInProgress)
    {
        m_isHandoverInProgress = false;
        std::cout << "================socket-base-receiveddata-m_isHandoverInProgress == true\n"
                  << "Data segment, seq=" << tcpHeader.GetSequenceNumber() << "\n"
                  << "pkt size=" << p->GetSize() << "\n" << "expectedSeq="  <<
    m_tcb->m_rxBuffer->NextRxSequence() << "\n"
                  << "m_highRxMark=" << m_highRxMark << "\n"
                  << "m_highTxAck=" << m_highTxAck << "\n"
                  << std::endl;
    }
    */
    ...
    { // NextRxSeq advanced, we have something to send to the app
        // 如果接收到的序列号大于预期序列号，说明接收缓冲区中有新数据可以传递给应用层
        // std::cout << "======expectedSeq < m_tcb->m_rxBuffer->NextRxSequence()==true=========" <<
        // std::endl;
        if (!m_shutdownRecv)
        {
            NotifyDataRecv();
    ...
    { // In-sequence packet: ACK if delayed ack count allows
        ...
        if (++m_delAckCount >= m_delAckMaxCount || m_isHandoverInProgress)
        {
            m_delAckEvent.Cancel();
            m_delAckCount = 0;
            m_congestionControl->CwndEvent(m_tcb, TcpSocketState::CA_EVENT_NON_DELAYED_ACK);
