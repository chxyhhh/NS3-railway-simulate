...
namespace ns3
{
    ...
    void TcpSocketBase::SetHandoverStatus(bool status)
    {
        m_isHandoverInProgress = status;
    }

    void TcpSocketBase::SetFeatureToggles(bool enableRwndShrink, bool enableAckSplit)
    {
        m_enableRwndShrink = enableRwndShrink;
        m_enableAckSplit = enableAckSplit;
    }

    void TcpSocketBase::SetAckSplitParams(uint32_t splitCount, double splitK)
    {
    ...
    if (isUE && (flags & TcpHeader::ACK) && !(flags & (TcpHeader::SYN | TcpHeader::FIN))
        && m_enableAckSplit && m_isHandoverInProgress && m_splitCount > 0)
    {
        // --- 专利算法：预测驱动动态 ACK 拆分 ---
        ...
        if (M > N)
        {
            ...
            for (uint32_t i = 1; i <= a; ++i)
            {
                ...
                Ptr<Packet> p = Create<Packet>();
                Simulator::Schedule(MicroSeconds(10 * i), [=]() {
                    m_tcp->SendPacket(p,
                                     header,
            ...
            else
            {
                ...
                if (!m_noDelay && UnAckDataCount() > 0 && availableData < m_tcb->m_segmentSize)
                {
                    NS_LOG_DEBUG("Invoking Nagle's algorithm for seq "
                                 ...
                                 << ". Wait to send.");
                    std::cout << "Invoking Nagle's algorithm for seq "
                                 << next << ", SFS: " << m_txBuffer->SizeFromSequence(next)
                                 << ". Wait to send." << std::endl;
                    break;
                }
    ...
    TcpSocketBase::AdvertisedWindowSize(bool scale) const
    {
        ...
        else
        {
            ...
            if (m_enableRwndShrink && m_rwndPhase != RWND_NORMAL) {
                double alpha  = 1.0;
    ...
    TcpSocketBase::ReceivedData(Ptr<Packet> p, const TcpHeader &tcpHeader)
    {
        ...
        if (m_isHandoverInProgress)
        {
            ...
            std::cout << "================socket-base-receiveddata-m_isHandoverInProgress == true\n"
                      ...
                      << "pkt size=" << p->GetSize() << "\n" << "expectedSeq="  << m_tcb->m_rxBuffer->NextRxSequence() << "\n"
        ...
        */
        ...
        {   // NextRxSeq advanced, we have something to send to the app
            ...
            // std::cout << "======expectedSeq < m_tcb->m_rxBuffer->NextRxSequence()==true=========" << std::endl;
            if (!m_shutdownRecv)
            {
                NotifyDataRecv();
            }
