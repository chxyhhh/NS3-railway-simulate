    ...
    static TypeId tid =
        TypeId("ns3::LteUeRrc")
            ...
            .AddTraceSource(
             ...
             .AddTraceSource("ReportUeMeasurement",
                    "Trace fired when UE sends a measurement report",
                    MakeTraceSourceAccessor(&LteUeRrc::m_reportUeMeasurementTrace),
                    "ns3::LteUeRrc::ReportUeMeasurementTracedCallback")
             .AddTraceSource("ReportUeMeasurementWithRawRsrp",
                             "Trace fired when UE sends a measurement report with raw RSRP values",
                             MakeTraceSourceAccessor(&LteUeRrc::m_reportUeMeasurementWithRawRsrpTrace),
                             "ns3::LteUeRrc::ReportUeMeasurementWithRawRsrpTracedCallback");
    return tid;
}
...
//        std::cout << "22222222222" << std::endl;
        m_reportUeMeasurementTrace (m_imsi, m_cellId, m_rnti, measurementReport);

        // 新增：传递原始 RSRP 值的 trace
        m_reportUeMeasurementWithRawRsrpTrace(m_imsi, m_cellId, m_rnti, measurementReport, servingMeasIt->second.rsrp, servingMeasIt->second.rsrq);
        ...
        /*
        std::cout << " reporting serving cell "
                            "RSRP "
                         << +measResults.measResultPCell.rsrpResult << " ("
                         << servingMeasIt->second.rsrp
                         << " dBm) "
                            "RSRQ "
                         << +measResults.measResultPCell.rsrqResult << " ("
                         << servingMeasIt->second.rsrq << " dB)" << std::endl;
                         */
