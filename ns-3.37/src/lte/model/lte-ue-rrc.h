...
class LteUeRrc : public Object
{
  ...
  public:
    ...
    typedef void (* ReportUeMeasurementTracedCallback)
                 (uint64_t imsi, uint16_t cellId, uint16_t rnti, const LteRrcSap::MeasurementReport& report);
    /**
     * TracedCallback signature for measurement report with raw RSRP values.
     */
    typedef void (*ReportUeMeasurementWithRawRsrpTracedCallback)(uint64_t imsi, uint16_t cellId, uint16_t rnti, const LteRrcSap::MeasurementReport& report, double rawRsrp, double rawRsrq);
  private:
    // PDCP SAP methods
    /**
    ...
    TracedCallback<uint64_t, uint16_t, uint16_t, LteRrcSap::MeasurementReport> m_reportUeMeasurementTrace;

    /**
     * The 'ReportUeMeasurementWithRawRsrp' trace source. Fired when UE sends a measurement report with raw RSRP values.
     */
    TracedCallback<uint64_t, uint16_t, uint16_t, LteRrcSap::MeasurementReport, double, double> m_reportUeMeasurementWithRawRsrpTrace;
